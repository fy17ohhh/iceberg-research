...
class LibraryManager:
    def __init__(
        ...
        self.pdf_reader_tool = pdf_reader_tool
    ...
    def get_doc_preview(self, title: str) -> dict:
        for entry in self.list_docs():
            ...
            if not converted_path or not os.path.isfile(converted_path):
                raise FileNotFoundError(f"Preview is unavailable for: {title}")
        ...
        raise KeyError(f"Document not found: {title}")
