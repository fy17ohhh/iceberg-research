...
from .prompts import SYNTHESIZER_SYSTEM_PROMPT, SYNTHESIZER_USER_PROMPT
...
class Synthesizer(AgentBase):
    ...
    def run(self, research_brief: str, notes: list[str]) -> str:
        ...
        synthesizer_response = self.llm.invoke(
            messages=messages,
            max_tokens=16384,
        ...
        report = self._resequence_citations(synthesizer_response.content)

        synthesizer_msg = Message(content=report, role="assistant")
        self._history.append(synthesizer_msg)
    ...
    def _resequence_citations(self, report: str) -> str:
        ...
        logger.info("[Synthesizer] resequence: %s → [1..%d]", list(old_to_new.keys()), len(old_to_new))
    ...
    def _renumber_citations(self, notes: list[str]) -> list[str]:
        ...
        logger.info(
            "[Synthesizer] citations: %d notes, %d unique sources (from %d total references)",
