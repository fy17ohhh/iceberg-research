...
function sanitizeMathDollars(text: string): string {
  // Split a single-line display formula across three lines so remark-math recognizes it
  const out = text.replace(/^\$\$(.+)\$\$$/gm, "$$$$\n$1\n$$$$");
  // Protect valid LaTeX pairs first, then escape remaining standalone currency dollar signs
  const saved: string[] = [];
  const masked = out
