...
export function resetNavigatorCache() {
...
export default function NavigatorPanel({
  ...
  useEffect(() => {
    _navigatorCache = { query, messages, settled };
  });
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    analyzeNavigation(query).then((data) => {
      if (cancelled) return;
    ...
    <div className="w-full max-w-2xl flex flex-col gap-4">
      {messages.map((msg) => {
        ...
        if (msg.role === "assistant-directions") {
          const hasMessage = msg.message.trim().length > 0;
          const hasDirections = msg.directions.length > 0;
          if (!hasMessage && !hasDirections) return null;
          return (
            <AssistantMsg key={msg.id}>
              {hasMessage && (
              ...
              {!settled && (revealDirections || !hasMessage) && (
                <>
                  <div className="flex flex-col gap-2.5">
                    {hasDirections && msg.directions.map((dir, j) => {
