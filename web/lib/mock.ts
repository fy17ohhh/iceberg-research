...
import type { NavigationResult, NavigationRefineResult, ResearchEvent, LibraryDoc, SaveReportResult } from "./api";
...
export function mockNavigation(_query: string): Promise<NavigationResult> {
...
export function mockNavigationRefinement(_query: string, _response: string): Promise<NavigationRefineResult> {
  return new Promise((resolve) => {
    setTimeout(() => {
...
export function startMockResearch(
  ...
  // 1. Plan — after 1.5s (navigation settled → brief shown → plan)
  schedule(1500, () => {
    onEvent({
      type: "navigator",
      sub_questions: [
  ...
  // 3. Review — after all research completes, briefly show the review shimmer
  schedule(16500, () => {
    onEvent({
      type: "sonar",
      round: 1,
  ...
  schedule(20000, () => {
    onEvent({ type: "synthesizer", report: MOCK_REPORT });
  });
