...
class Pipeline:
    ...
    VECTOR_STORE_MIGRATION_MARKER = ".qdrant_sqlite_migration_complete"
    ...
    def __init__(
        ...
        legacy_store_path = os.path.join(data_dir, self.VECTOR_STORE_FILE)
        migration_marker = os.path.join(
            data_dir, self.VECTOR_STORE_MIGRATION_MARKER
        )
        if os.path.exists(legacy_store_path) and not os.path.exists(migration_marker):
            migrated = self.vector_store.migrate_json(legacy_store_path)
            with open(migration_marker, "w", encoding="utf-8") as marker:
                marker.write(
                    f"source={legacy_store_path}\n"
                    f"migrated_chunks={migrated}\n"
                )
    ...
    def save(self):
        # Qdrant and SQLite commit on every mutation. Keep this method so
        # LibraryManager and existing callers do not need storage-specific code.
